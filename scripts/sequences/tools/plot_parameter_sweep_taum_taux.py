...
divnorm = colors.LogNorm(vmin=parspace[:11,:].min(),vmax=200)
...
plt.xticks(np.arange(len(tau_x_sweep))[::5],tau_x_sweep[::5])
fig.tight_layout(rect=[0, 0.01, 1, 0.97])
plt.xlim(0,11)
plt.colorbar()
plt.xlabel(r'$\tau_{x}$')
plt.ylabel(r'$\tau_{m}$')
plt.savefig(savedir+'parameter_space_taum_taux.png',format='png', dpi=300)
plt.savefig(savedir+'parameter_space_taum_taux.pdf',format='pdf', dpi=300)
plt.close('all')

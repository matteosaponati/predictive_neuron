...
dt_burst, dt = np.array([1000,100,50,20])/par.dt, 10

'set inputs'
timing = np.array([2.,2+dt])/par.dt
E_pre, E_post = [], []
w1_pre,w2_pre = [],[]
w1_post,w2_post = [],[]
v_pre, spk_pre = [],[]
...
for k in dt_burst:
    ...
    w_0_pre = torch.Tensor([.01,.1])
    neuron = models.NeuronClass(par)
    neuron.w = nn.Parameter(w_0_pre.clone()).to(par.device)
    'set inputs'
    timing = [np.arange(0,k*5,k),np.arange(0,k*5,k)+dt/par.dt]
...
w_pre = [w1_pre[k][-1] for k in range(len(dt_burst))]
w_post = [w2_post[k][-1] for k in range(len(dt_burst))]
...
#'set timing'
#spk_times = [(np.linspace(par.Dt,par.Dt*par.N,par.N)/par.dt).astype(int),
#             (np.linspace(par.Dt,par.Dt*par.N,par.N)/par.dt).astype(int)[::-1]]
#timing = [[] for n in range(par.nn)]
#for n in range(par.nn):
#    for b in range(par.batch): 
#        timing[n].append(spk_times[b])
...
for n in range(par.nn):
    ...
    plt.plot(network[n].w[np.argsort(timing[0])[0]],'r')
    plt.plot(network[n].w[np.argsort(timing[0])[1]],'b')
...
for n in range(par.nn):
    w_final[:,n] = w_plot[-1,:,n]

plt.figure(figsize=(8,6))
'comparison for different batches'
for b in range(par.batch):
    plt.subplot(1,2,b+1)
    plt.imshow(w_final[np.argsort(timing[0])[b]],aspect='auto');
    plt.colorbar()

#%%
'get v'

v_plot = np.zeros((2,par.epochs,par.nn,par.T))
...
n = 3
e = -2
plt.plot(v_plot[0,e,n,:],'r')
plt.plot(v_plot[1,e,n,:],'b')
#%%

spk_plot = [[[] for b in range(par.batch)] for n in range(par.nn)] 

for e in range(par.epochs):
    for n in range(par.nn):
        
        spk_plot[n][0].append(spk[e][n][0])
        spk_plot[n][1].append(spk[e][n][1])
        
#%%
n = 1
#fig = plt.igure(figsize=(7,6), dpi=300)
for k,j in zip(spk_plot[n][0],range(par.epochs)):
    plt.scatter([j]*len(k),np.array(k),c='r',s=2)
for k,j in zip(spk_plot[n][1],range(par.epochs)):
    plt.scatter([j]*len(k),np.array(k),c='b',s=2)

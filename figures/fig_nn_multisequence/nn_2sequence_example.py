"""
n_in=2, nn=2, T=300, epochs=400, Dt=4, tau=10, vth=2.5, w_0=.1, w_0rec=-.05
"""
import numpy as np
import torch
import types
...
def forward(par,network,x_data,online=False):
    ...
    v,z = [], [[[] for b in range(par.batch)] for n in range(par.nn)]
    ...
    for t in range(par.T):            
        ...
        for n in range(par.nn):
            for b in range(par.batch):
                if network.z[b,n] != 0: z[n].append(t*par.dt)  
...
for n in range(par.nn):
    ...
    timing[n].append(np.arange(0,par.Dt*par.n_in,par.Dt)[::-1]/par.dt)
...
plt.plot(w[:,0,0],color='purple',linewidth=2)
plt.plot(w[:,0,1],color='navy',linewidth=2)
plt.plot(w[:,1,0],color='purple',linewidth=2,linestyle='dashed')
plt.plot(w[:,1,1],color='navy',linewidth=2,linestyle='dashed')
plt.xlabel('epochs')
plt.ylabel(r'$\vec{w}$')

#%%
plt.plot(np.arange(0,par.T*par.dt,par.dt),v_out[-1][1,:,0].detach().numpy(),linewidth=2,color='purple')
plt.plot(np.arange(0,par.T*par.dt,par.dt),v_out[-1][1,:,1].detach().numpy(),linewidth=2,color='navy')

...
def train(par):
    ...
    for e in range(par.epochs):
        ...
        x_data[mask<prob] = 1        
        x_data[:,timing,range(par.N_seq)] = 1
...
#    par.device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    np.save('loss_seq_Ndist_{}'.format(par.N_dist),loss)
    np.save('v_seq_Ndist_{}'.format(par.N_dist),v)
    np.save('w_seq_Ndist_{}'.format(par.N_dist),w)
    np.save('spk_seq_Ndist_{}'.format(par.N_dist),spk)
...
if __name__ == '__main__':
    ...
    par.savedir = '/mnt/pns/departmentN4/matteo_data/predictive_neuron/pattern_generalization/'
#    par.device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    np.save('loss_pattern',loss)
    np.save('v_pattern',v)
    np.save('w_pattern',w)
    np.save('spk_pattern',spk)
    ...
    if par.offset == 'True':    
        np.save('loss_offset_rep_{}'.format(par.rep),loss)
        np.save('v_offset_rep_{}'.format(par.rep),v)
        np.save('w_offset_rep_{}'.format(par.rep),w)
        np.save('spk_offset_rep_{}'.format(par.rep),spk)
